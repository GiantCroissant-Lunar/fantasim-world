...
class InboxOrganizer:
    ...
    def __init__(self, inbox_path: Path, dry_run: bool = False):
        ...
        self.categorization_rules: Dict[str, Callable[[str], bool]] = {
            'conversation-exports': lambda f: (f.startswith('2026-') and f.endswith('.txt')) or ('caveat' in f.lower()),
            'discussions': lambda f: 'discussion' in f.lower(),
            'implementations': lambda f: any(x in f.lower() for x in ['implement', 'rfc-v2', 'contracts', 'dataset']) or ('merged' in f.lower() and ('implement' in f.lower() or 'rfc' in f.lower())),
            'refactoring': lambda f: 'refactor' in f.lower(),
            'integrations': lambda f: any(x in f.lower() for x in ['integrate', 'opencode', 'ui_ux']),
            'planning': lambda f: any(x in f.lower() for x in ['research', 'plan', 'alignment', 'gplates']),
            'maintenance': lambda f: any(x in f.lower() for x in ['merge', 'fixing', 'clean', 'unload', 'sync', 'sync-']),
        }
    ...
    def organize_files(self):
        ...
        files = [f for f in self.inbox_path.iterdir() if f.is_file() and f.name != 'README.md' and f.name != '.gitignore']
    ...
    def generate_readme(self):
        ...
        lines.append("- `task docs:merge-duplicates`: Intelligently merges overlapping content and removes redundancy.\n")
        lines.append("- `task docs:organize-inbox`: Organizes files into the folders defined above.\n")
