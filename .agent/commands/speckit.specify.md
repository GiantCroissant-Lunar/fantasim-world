...
2. **Check for existing branches before creating new one**:
   ...
   a. Create the numbered feature branch + worktree by running `task spec:new -- <short-name>` from repo root.
      ...
      ```powershell
      task spec:new -- user-auth
      ```

   b. `cd` into the created worktree and scaffold the spec file:

      ```powershell
      task spec:specify FEATURE=<branch-name>
      ```

   **IMPORTANT**:
   - Treat the branch name printed by `task spec:new` as the feature ID (example: `001-user-auth`).
   - Do not create numbered branches/worktrees manually.
